...
def quartet_metabolite_report_execution_start():
  ...
  if 'conclusion/cutoff_table' not in config.sp:
    config.update_dict( config.sp, { 'conclusion/cutoff_table': { 'fn_re': '^cutoff_table.csv$' } } )
  
  # Module-snr
  if 'snr/table_met' not in config.sp:
    config.update_dict( config.sp, { 'snr/table': { 'fn_re': '^PCAtable.csv$' } } )
  
  # Module-correlation
  if 'correlation/table_met' not in config.sp:
    config.update_dict( config.sp, { 'correlation/table': { 'fn_re': '^RCtable.csv$' } } )
